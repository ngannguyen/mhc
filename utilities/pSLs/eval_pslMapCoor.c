...
/*
 *This script maps coordinates of input PSLs (from a file) to their locations on the original genome sequences.
 */
...
void mapPslCoor(struct psl *pslRecord, int32_t qrefsize, int32_t qstart, int32_t qlen, int32_t trefsize, int32_t tstart, int32_t tlen){
   ...
   pslRecord->qStart += qstart;
   pslRecord->qEnd += qstart;
   pslRecord->tStart += tstart;
   pslRecord->tEnd += tstart;
   ...
   for(i=0; i< pslRecord->blockCount; i++){
      *(pslRecord->qStarts + i) = qstart + *(pslRecord->qStarts +i);
      *(pslRecord->tStarts + i) = tstart + *(pslRecord->tStarts +i);
   }
   return;
}
...
void usage() {
   ...
   fprintf(stderr, "Convert coordinates of input psls to their coordinates on the original genome sequences.\n");
   ...
   fprintf(stderr, "-q --query location on its reference genome (forward strand): chrSize.qStart.qLength. E.g: 171115067.31321610.3479\n");
   fprintf(stderr, "-t --target location on its reference genome (forward strand): chrSize.tStart.tLength.\n");
